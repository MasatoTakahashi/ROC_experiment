...
ui <- dashboardPage(
    ...
    dashboardSidebar(
        ...
        sliderInput('n_total_case', label='N total case', min=100, max=10000, value=1000, step=100),
    ...
    dashboardBody(
        fluidRow(
            ...
            box('Grand truth ratio', plotOutput('graphs0', height='80px'), width='100%'),
...
server <- function(input, output) {
    ...
    d <- reactive({
        ...
        y_positive <- rnorm(n_positive_case, mean = input$score_mean_pos)
        y_negative <- rnorm(n_negative_case, mean = input$score_mean_neg)
    ...
    output$optimal_threshold_ui <- renderText({
        ret <- glue('Optimal threshold with Youden Index = {round(optimal_threshold(), 6)}<br><br>')
        return(ret)
    })
    ...
    output$graph_confusion_matrix2 <- renderPlot({
        g <- get_confusion_matrix_plot(d(), th=optimal_threshold(), 'Confusion Matrix(threshold: Youden Index)')
        return(g)
    }, bg="transparent")
    
    output$graphs_score_hist1 <- renderPlot({
        g <- plot_score_histogram(d(), th=input$threshold, n_bins=input$n_bins, opt_th=optimal_threshold())
        return(g)
    }, bg="transparent")
    
    output$graphs_score_hist2 <- renderPlot({
        g <- plot_score_histogram(d(), th=input$threshold, n_bins=input$n_bins, opt_th=optimal_threshold(), mode='density')
        return(g)
    }, bg="transparent")
